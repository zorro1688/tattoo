...
await run("legal pages provide required product information and static routes", async () => {
  ...
  for (const [htmlFile, route, heading, commitments] of pages) {
    ...
    assert.match(html, /support@inkfirsttattoo\.art/);
...
await run("Next routes render the corresponding legal documents", async () => {
  for (const [pageFile, htmlFile] of [
    ...
    ["app/acceptable-use/page.tsx", "acceptable-use.html"]
  ]) {
...
await run("landing footer links to the legal pages and uses a copyright entity", async () => {
  ...
  assert.match(home, /href="\/acceptable-use">Acceptable Use Policy<\/a>/);
  assert.match(home, /mailto:support@inkfirsttattoo\.art/);
  assert.doesNotMatch(home, /hello@inkfirst\.ai/);
  assert.match(home, /&copy; 2026 InkFirst\. All rights reserved\./);
  assert.doesNotMatch(home, /\? 2026 InkFirst/);
});

await run("all public legal contact addresses match the Creem support email", async () => {
  for (const htmlFile of ["privacy.html", "terms.html", "refunds.html", "acceptable-use.html"]) {
    const html = await readFile(htmlFile, "utf8");
    assert.match(html, /support@inkfirsttattoo\.art/);
    assert.doesNotMatch(html, /hello@inkfirst\.ai/);
  }
});
